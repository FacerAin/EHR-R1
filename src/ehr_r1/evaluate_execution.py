...
def main():
    ...
    parser.add_argument(
        "--predictions_file",
        required=True,
        help="JSON file with predicted SQL queries"
    )
    parser.add_argument(
        "--ground_truth_file", 
        required=True,
    ...
    try:
        ...
        print(f"Predicted success rate: {results.get('predicted_success_rate', 'N/A'):.4f}")
        print(f"Ground truth success rate: {results.get('ground_truth_success_rate', 'N/A'):.4f}")
