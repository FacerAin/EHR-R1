...
class EHRSQLEvaluator:
    ...
    def compute_execution_accuracy(
        ...
        if not self.execution_evaluator:
            raise ValueError(
                "Database path not provided. Cannot compute execution accuracy."
            )

        return self.execution_evaluator.evaluate_batch(
            predicted_queries, ground_truth_queries
        )
    ...
    def evaluate(
        ...
        results["exact_match_accuracy"] = self.compute_exact_match(
        ...
        results["empty_prediction_rate"] = 1 - (
            results["non_empty_predictions"] / len(predictions)
        )
