    ...
    def _clean_query(self, query: str) -> str:
        ...
        for keyword in dangerous_keywords:
            if keyword in query_upper:
                print(
                    f"Warning: Potentially dangerous keyword '{keyword}' found in query"
                )
    ...
    def normalize_result(self, result: Any) -> Any:
        ...
        if isinstance(result, list):
            ...
            if result and isinstance(result[0], list):
                ...
                try:
                    return sorted(
                        [str(val) if val is not None else None for val in result]
                    )
                except TypeError:
...
class ExecutionAccuracyEvaluator:
    ...
    def evaluate_single(
        ...
        gt_success, gt_result, gt_error = self.sql_executor.execute_query(
            ground_truth_query
        )
        ...
        if gt_success and pred_success:
            results["execution_match"] = self.sql_executor.compare_results(
